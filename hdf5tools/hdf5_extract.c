...
static herr_t CopyObject (hid_t from,
                          ...
                          void *_to)
{
  hid_t to, datatype, dataspace, from_group, to_group, lcpl_id;
  ...
  else if (objectinfo.type == H5G_DATASET)
  {
    ...
    CHECK_ERROR (lcpl_id = H5Pcreate(H5P_LINK_CREATE));
    CHECK_ERROR (H5Pset_create_intermediate_group(lcpl_id, 1));
    CHECK_ERROR (to = H5Dcreate2 (to, objectname, datatype, dataspace,
                                 lcpl_id, H5P_DEFAULT, H5P_DEFAULT));
    CHECK_ERROR (H5Pclose(lcpl_id));
